...
class Codegen final : public Visitor {
    ...
    enum Phase : u8 { Declare, Define } m_phase; 
    Options& m_opts;
    Root& m_root;
    siir::CFG& m_cfg;
    ...
    std::unordered_map<Decl*, std::string> m_mangled;

    /// Mangle the name of \p decl as per its runes, and cache it for later
    /// references.
    const std::string& mangle(Decl* decl);
    ...
    /// arguments. 
    void lower_function(FunctionDecl* decl);
    
    /// Implement the body of function \p decl.
    void impl_function(FunctionDecl* decl);
    
    /// Lower the structure \p decl to a new SIIR StructType. 
    void lower_structure(StructDecl* decl);
